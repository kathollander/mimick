 ...
 * test paper (sample/test-paper.pdf). Checks the regions are drawn and numbered, that a click skips one
...
await r.click(await r.at(290, 260)); await sleep(2000);
s = await state();
const skipped = await sentences();
check("clicking a region skips it", /Now skipping that region/.test(s.status) && skipped < 29, s.status);
check("…draws it as skipped", s.reads === 5 && s.skipped === 3, [s.reads, s.skipped]);
check("…does not start reading", s.play === "Read aloud", s.play);
check("…and the highlight is still on the same words", s.card.includes("voice keeps the pace"), s.card);

// 3. Kept.
await r.load();
await r.openPdf(SAMPLE); await sleep(1500);
s = await state();
check("after a reload the order shows, with the change put back", s.reads === 5 && s.skipped === 3, [s.reads, s.skipped]);
await display();
...
await display(); await r.menu("Clean up text for reading"); await sleep(2500);
check("switching the cleanup off and on keeps the change", (await sentences()) === skipped && (await state()).skipped === 3, await r.status());

// 5. Reset.
await display();
await r.menu("Reset reading order"); await sleep(2500);
s = await state();
check("Reset puts the analysis back", /reset/.test(s.status) && s.reads === 6 && s.skipped === 2, [s.status, s.skipped]);
await r.load();
await r.openPdf(SAMPLE); await sleep(1500);
s = await state();
await display();
check("…and forgets the change", s.reads === 6 && (await r.menuLabels())?.includes("(off) Reset reading order"), [s.reads, s.skipped]);
await r.key("Escape");
