 ...
 * read starting off, then on and off again, Clean up text for reading rebuilding the sentences (29
...
check("Click to read starts off: a click on a sentence does not read", (await play()) === "Read aloud", await play());
await display("Click to read");
check("switching Click to read on says so", /Click any sentence to read/.test(await r.status()), await r.status());
check("…and remembers it", (await ev(`localStorage.getItem("mimick-click_read")`)) === "1");
await r.click(await r.at(290, 260));
await wait(`document.getElementById("play").textContent === "Pause"`, 60000);
check("…and a click on a sentence then reads", true);
await r.key(" ");
await wait(`document.getElementById("play").textContent === "Resume"`);
await display("Click to read");
check("switching it off again says so", /Clicking never starts reading/.test(await r.status()), await r.status());
...
check("the switches show what is on", (await pressed("sw-clean_text")) === "true" && (await pressed("sw-click_read")) === "false", [await pressed("sw-clean_text"), await pressed("sw-click_read")]);
await r.click(await r.centre("#sw-click_read")); await sleep(400);
check("clicking one switches it", (await pressed("sw-click_read")) === "true" && (await ev(`localStorage.getItem("mimick-click_read")`)) === "1");
await display();
check("…and Reading ▾ agrees", await ev(`[...document.querySelectorAll("#menu button")].find((b) => b.textContent.includes("Click to read")).getAttribute("aria-checked") === "true"`));
await r.menu("Click to read"); await sleep(400);
check("…and the other way round", (await pressed("sw-click_read")) === "false");
