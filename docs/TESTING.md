...
## The voice's download, 20 September — https://kathollander.github.io/mimick/reader.html

Only once the 20 September work is pushed; it is committed but not deployed.
A real first visit again (cleared site data, or a private window).

- [ ] When Mimick says Ready, a small bar in the bottom corner says **Downloading Norman's voice… N of 97 MB**
- [ ] It does not cover the page, and a document can be opened and read on screen while it counts up
- [ ] Press **Read aloud** while it is still counting: the status line follows the same number, and reading starts when it reaches the end (it does not download twice)
- [ ] When it finishes, the status line says everything is kept for offline use
- [ ] Then turn off the wi-fi, reload, open a document and press Read aloud: it reads
- [ ] How long from opening the page to the voice bar disappearing: ____ (Firefox) ____ (Edge)
