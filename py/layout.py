...
# Columns taller than this share of what they stand in are read down, one after
# the other, even where their paragraph breaks happen to line up.
COLUMN_TALL_SHARE = 0.6
...
def _gaps(blocks: list, axis: int, minimum: float):
    ...
    def counts(i: int) -> bool:
        gap = merged[i + 1][0] - merged[i][1]
        if gap >= minimum:
            return True
        return (axis == 0 and gap >= MIN_GUTTER_X
                and merged[i][1] - merged[i][0] >= MIN_COLUMN_WIDTH
                and merged[i + 1][1] - merged[i + 1][0] >= MIN_COLUMN_WIDTH)

    candidates = [(merged[i + 1][0] - merged[i][1], i)
                  for i in range(len(merged) - 1) if counts(i)]
    if not candidates:
        return None
    widest, index = max(candidates)
...
def _cut(blocks: list, prefer_rows: bool, depth: int = 0) -> list[list]:
    ...
    # Side-by-side columns that each run most of the way down are cut apart
    # before anything else. Otherwise two columns that happen to break a
    # paragraph at the same height are cut across there, and the page reads
    # the top of one column, the top of the next, then back to the first.
    # Nothing that spans the gutter -- a title, a table -- lets this cut
    # happen at all, so those still get their rows first.
    columns = _gaps(blocks, 0, MIN_GAP_X)
    if columns is not None:
        _width, before, after = columns
        top = min(b[1] for b in blocks)
        height = max(b[3] for b in blocks) - top
        if height > 0 and all(
            max(b[3] for b in side) - min(b[1] for b in side)
            >= height * COLUMN_TALL_SHARE for side in (before, after)
        ):
            return _cut(before, True, depth + 1) + _cut(after, True, depth + 1)
...
def analyse(document, skip_references: bool = True) -> dict[int, list[Region]]:
    ...
    for number, blocks in pages.items():
        ...
        # On a page of several columns, a heading in the next column over is
        # narrow and beside the main text too. What gives it away is that it
        # starts where a full-width block of text in another column starts: it
        # is in a column, not in the margin. (Only blocks clear of the main
        # text count, or a journal's title, which spans the page, would vouch
        # for the sidebar that shares its left edge.)
        column_edges = [] if main is None else [
            group["rect"][0] for group in laid_out
            if group["rect"][2] - group["rect"][0]
            >= (main["rect"][2] - main["rect"][0]) * ASIDE_WIDTH_SHARE
            and (group["rect"][2] <= main["rect"][0] + 2
                 or group["rect"][0] >= main["rect"][2] - 2)
        ]
        for group in laid_out:
            ...
            if main is not None and group is not main:
                ...
                in_column = any(abs(group["rect"][0] - edge) <= 2
                                for edge in column_edges)
                if beside and not in_column and width < main_width * ASIDE_WIDTH_SHARE:
                    kind = ASIDE
