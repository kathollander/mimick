...
lit on the page. You can read from any sentence (a click, once Click to read is
on), jump around by
...
time, and it is under **Help ▾** afterwards. The tour is still a work in
progress.
...
4. **Select some text and press `Enter`** to read just that, or right-click →
   **Start reading from here**. Turn on Reading ▾ → **Click to read** and a
   click on any sentence reads from there; it starts off, so that a click can
   select and highlight without the voice jumping. `←` and `→` go back and forward a sentence.
