 ...
 *                                     the app, and the engine that reads with it,
 *                                     if they are not kept already, and ask the
 *                                     browser not to clear what is kept
 *   MimickVoicePicker.watchDefault(fn)   fn({ busy, loaded, total }) now and as
 *                                     keepDefault downloads: bytes as read
 */
(function (root) {
  "use strict";
   ...
   * always a voice, offline too. With it, the engine that reads with any voice
   * -- ONNX Runtime and the phonemizer, which js/piper-worker.js loads --
   * fetched through sw.js, which keeps them as they pass. Part of the first
   * visit's download: the page's bar shows it once Python is up, and Read
   * aloud waits for it rather than fetch a second copy. The rest of the
   * offline copy waits for it too (reader.js). Resolves to true when it is kept. */
  const ENGINE = ["vendor/onnxruntime/ort.wasm.min.js", "vendor/onnxruntime/ort-wasm-simd-threaded.mjs",
                  "vendor/onnxruntime/ort-wasm-simd-threaded.wasm", "vendor/piper/piper_phonemize.js",
                  "vendor/piper/piper_phonemize.wasm", "vendor/piper/piper_phonemize.data"];
  // The engine's files and Norman's two, as read (after any gzip), for the bar.
  // If they change the bar is only a little off, and never past full.
  const KEEP_BYTES = 96.7e6;
  const progress = { busy: false, loaded: 0, total: KEEP_BYTES };
  const watchers = new Set();
  const tell = (patch) => {
    Object.assign(progress, patch);
    for (const fn of watchers) fn({ ...progress });
  };
  function watchDefault(fn) { watchers.add(fn); fn({ ...progress }); }

  async function keepEngine(onBytes) {
    for (const path of ENGINE) {
      ...
      // Read to the end, so the service worker has all of it to keep.
  ...
  function keepDefault() {
    return keeping ??= (async () => {
      // Asked, never waited on: Firefox answers only once the reader has
      // clicked through a prompt, and until then the voice never loaded -- the
      // page sat at "Getting the voice ready…" (found 24 September). Keeping
      // the voice does not need the answer; it only makes it harder to evict.
      try { navigator.storage?.persist?.().catch(() => {}); } catch { /* the browser decides */ }
      let engine = 0;
      try {
        await keepEngine((n) => tell({ loaded: engine += n }));
        if (!(await Voices.kept(Voices.DEFAULT))) {
          tell({ busy: true });
          await Voices.fetchVoice(Voices.DEFAULT, (loaded) => tell({ loaded: engine + loaded }));
        }
        tell({ busy: false, loaded: KEEP_BYTES });
        return true;
      } catch (err) {
        console.warn("Mimick: the default voice could not be kept yet:", err);
        tell({ busy: false });
        keeping = null;
        return false;
      }
  ...
  root.MimickVoicePicker = { create, keepDefault, watchDefault };
})(window);
