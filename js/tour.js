 ...
 * says so the moment the reader does the thing, and then waits for Next too.
 * It used to move on by itself, which left no time to see what had happened
 * -- or moved on before the page had finished doing it (Kat, 23 September).
 *
...
(function (root) {
  ...
  function create(ctx) {
    ...
    let timer = null, ghostTimer = null;
    ...
    const STEPS = [
      {
        ...
        text: "A couple of minutes on a sample poem, so you can try everything before you open anything of your own. "
            ...
            + "The tour is still a work in progress, so some steps may change.",
      ...
      },
      {
        ...
        praise: "There it goes — Ctrl+B again brings it back.",
      },
      // Click to read comes after the selecting and highlighting on purpose:
      // it starts off, so a click there selects rather than starting the voice.
      {
        title: "Click to read: turn it on",
        text: "Clicking a sentence can start the voice from there. It starts off, so that clicks select and highlight. "
            + "Turn it on with the Click to read switch here, or Reading ▾ → Click to read.",
        spotlight: "#sw-click_read",
        gesture: "tap",
        before: () => ctx.showSwitches(),
        done: (now) => now.clickRead,
        praise: "On.",
      },
      {
        title: "Start anywhere",
        text: "Now click any sentence, and it reads from there.",
        spotlight: "sentence",
        gesture: "tap",
      ...
      {
        title: "And off again",
        text: "The same switch turns it off, and a click only selects again. Every switch in this row works that way: "
            + "click once for on, again for off. Leave Click to read on if you like it.",
        spotlight: "#sw-click_read",
        gesture: "tap",
        before: () => ctx.showSwitches(),
        done: (now) => !now.clickRead,
        praise: "Off — clicks select again.",
      },
      {
    ...
    function tick() {
      ...
      // Done: Skip becomes Next, and the step waits for it.
      const go = $("tour-go");
      go.textContent = "Next";
    ...
    function stop({ finished = false } = {}) {
      if (at < 0) return;
      clearInterval(timer); clearInterval(ghostTimer);
      timer = ghostTimer = null;
      at = -1;
      spot = null;
      layer.hidden = true;
