...
(function () {
  ...
  function startWorker(url, onReady) {
    ...
    const self = {
      ...
      /* `onProgress(done, total)`, if given, hears how far a long job has got
       * before the answer comes. It is not the answer, so it never settles. */
      ask(message, transfer = [], onProgress = null) {
    ...
    worker.onmessage = ({ data }) => {
      ...
      if (!waiting) return;
      // How far a long job has got, not its answer: the asker is still waiting.
      if (data.type === "progress") { waiting.onProgress?.(data.done, data.total); return; }
  ...
  const readyNow = () => {
    ...
    // Python is loaded, and kept as it came. Next the default voice and its
    // engine, which ship with the app -- always one voice, offline too -- and
    // only then the rest of the offline copy, so the two do not share a slow line.
    MimickVoicePicker.keepDefault().then((kept) => {
      if (kept) fillVoices();
  ...
  // A file made for the reader to keep, where the browser has no save window, so
  // it downloads. A browser can block a download the page starts by itself --
  // Brave and Chrome do, once a tab has downloaded before -- and never tell the
  // page. So the file also waits in the bottom bar, as a link for the reader's
  // own click, until the next one or another document.
  let offered = null;
  function offer(name, blob) {
    if (offered) URL.revokeObjectURL(offered);
    offered = URL.createObjectURL(blob);
    const link = Object.assign($("status-file"), {
      href: offered, download: name, hidden: false, textContent: "Not in your downloads?",
      title: `Download ${name} again. If it still does not arrive, the browser has blocked it: `
           + "look for a blocked-download icon at the end of the address bar, and allow downloads from this site.",
    });
    link.click();
  }
  function withdraw() {
    if (offered) URL.revokeObjectURL(offered);
    offered = null;
    Object.assign($("status-file"), { hidden: true, href: "" });
  }

  // Something the status line suggests, as a real button beside it: "Save a
  // copy now", "Recognise the text". One at a time; `owner` names who asked,
  // so only they take it away again -- or opening another document does.
  let asking = null;
  function ask(owner, label, title, run) {
    asking = owner;
    Object.assign($("status-do"), { hidden: false, textContent: label, title, ariaLabel: label });
    $("status-do").onclick = () => { unask(owner); run(); };
  }
  function unask(owner = null) {
    if (owner !== null && asking !== owner) return;
    asking = null;
    $("status-do").hidden = true;
    $("status-do").onclick = null;
  }
  ...
  // { name, serial, converting?: { done, total }, slides?, preparing?, drawing? }
  // while a document opens. `converting` is the stretch before there is a PDF at
  // all -- a file being laid out as one -- which for a slide deck is most of the
  // wait. `serial` keeps the bar one bar across that and the opening that
  // follows, rather than letting it drop back to nothing in between.
  let loading = null;
  let openSerial = 0;
  ...
  let voiceKeep = { busy: false, loaded: 0, total: 1 };   // MimickVoicePicker.watchDefault
  ...
  function showLoading() {
    ...
    if (loading && !loading.converting && !loading.preparing && !loading.drawing) loading = null;
    ...
    if (starting) {
    ...
    } else if (loading?.converting) {
      // Laying the file out as a PDF: the half of the wait that used to show
      // nothing. A deck counts its slides as they are drawn; the other kinds
      ...
      // A file that was laid out first is already half-way along the bar.
      if (loading.laidOut) target = 0.5 + 0.5 * target;
    ...
    } else if (voiceKeep.busy && !stuck) {
      // The rest of the first visit: the reader works meanwhile, so the bar
      // steps aside to the corner, and moves only with the bytes.
      phase = "voice";
      label = `Downloading ${Voices.byKey[Voices.DEFAULT].name}'s voice…`;
      target = Math.min(1, voiceKeep.loaded / voiceKeep.total);
      creep = 0;
      detail = `${Math.round(voiceKeep.loaded / 1e6)} of ${Math.round(voiceKeep.total / 1e6)} MB, once. `
        + "Open a document meanwhile; after this, Mimick reads aloud with no internet connection.";
    }
    if (phase !== barPhase) { barPhase = phase; barShown = 0; barSince = performance.now(); }
    const bar = $("loading");
    bar.hidden = !phase;
    bar.classList.toggle("aside", phase === "voice");
  ...
  async function openBytes(buffer, name, key = null) {
    ...
    // A file that was laid out first keeps that bar rather than starting a
    // second one: the reader chose one file and should see one bar.
    const laidOut = loading?.converting ? loading : null;
    loading = { name, serial: laidOut?.serial ?? ++openSerial, laidOut: !!laidOut,
                preparing: true, drawing: true };
    $("empty").hidden = true;
    voice.open(0);
    closeMenu();
    notes.close();
  ...
  /* A scan recognises its own text as it opens (Kat, 23 September), when it
   * is short: about 2 s a page here, and the result reopens the document, so a
   * long book would be twenty minutes of work nobody asked for that ends by
   * pulling the page out from under its reader. A long one gets a button. */
  const AUTO_OCR_PAGES = 20, OCR_SECONDS_A_PAGE = 2;
  function offerRecognising() {
    const n = doc.textless.length;
    if (n <= AUTO_OCR_PAGES) { recogniseText(); return; }
    const m = Math.max(1, Math.round(n * OCR_SECONDS_A_PAGE / 60)), minutes = m === 1 ? "a minute" : `${m} minutes`;
    // Into openedStatus, which the bottom bar shows again as the pages finish drawing.
    openedStatus = `${openedStatus.split(" · ")[0]} · ${n} pages are pictures with no text. Recognising the words takes about ${minutes}, `
  ...
  // Laid out as PDFs by reader.document_to_pdf. The same kinds as CONVERTIBLE in
  // py/convert.py, which is the one that decides: a kind listed here and not
  // there is offered and then refused.
  const LAID_OUT = { docx: "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                     ...
                     xhtml: "application/xhtml+xml", rtf: "application/rtf",
                     fb2: "application/x-fictionbook+xml",
                     pptx: "application/vnd.openxmlformats-officedocument.presentationml.presentation",
                     ppsx: "application/vnd.openxmlformats-officedocument.presentationml.slideshow",
                     odp: "application/vnd.oasis.opendocument.presentation" };
  // The kinds that are another kind under a second name, as py/convert.py's
  // ALIASES has them. Only used for the key a document's notes hang off, so
  // that the same file opened as .htm and as .html keeps one set of them.
  const SAME_AS = { htm: "html", xhtml: "html", ppsx: "pptx" };
  // The name decides, and the type only answers for a file that has no useful
  // name: a .md handed over as text/plain is a Markdown file, not a text file,
  // and laying it out as plain text would lose every heading it has.
  const laidOutKind = (file) => {
    const suffix = (file.name.toLowerCase().match(/\.([a-z0-9]+)$/) ?? [])[1];
    if (suffix && suffix in LAID_OUT) return suffix;
    if (suffix) return null;
    return Object.keys(LAID_OUT).find((kind) => file.type && file.type === LAID_OUT[kind]) ?? null;
  };
   ...
   * A slide deck is drawn a page per slide and takes a few seconds, which the
   * worker does off the page's own thread, so the window stays answerable. */
  async function openDocument(file, kind) {
    const bytes = await file.arrayBuffer();
    const mine = ++generation;
    status(pythonReady ? `Laying out ${file.name}…` : `Getting ready, then opening ${file.name}…`);
    laying(file.name, SLIDE_KINDS.has(kind));
    const stem = file.name.replace(/\.[^.]+$/, "");
    let pdf;
    try {
      pdf = await callWatching((done, total) => {
        if (mine !== generation || !loading?.converting) return;
        loading.converting = { done, total };
        showLoading();
      }, "document_to_pdf", new Uint8Array(bytes), kind, stem);
    } catch (err) {
      if (mine !== generation) return;
      loading = null;
      showLoading();
      status(`Could not open ${file.name}: ${err.message.trim().split("\n").pop()}`);
      return;
    }
    if (mine !== generation) return;
    openBytes(pdf.slice().buffer, stem + ".pdf", `${SAME_AS[kind] ?? kind}:` + await Store.hash(bytes));
  }
  ...
  const voice = MimickReadAloud.create({
    ...
    onState(state) {
      ...
      // From here the voice is making the first sentence, which takes a few seconds.
      if (state === "buffering" && voiceWas === "loading") status("Starting to read…");
      voiceWas = state;
    ...
    onStatus: status,
    // Norman comes down with the app, just after Python; reading waits for that
    // download rather than start a second one.
    beforeLoad: (key) => (key === Voices.DEFAULT ? MimickVoicePicker.keepDefault() : null),
  });
  MimickVoicePicker.watchDefault((progress) => {
    voiceKeep = progress;
    if (voice.state === "loading") {
      status(progress.busy ? `Downloading ${Voices.byKey[Voices.DEFAULT].name}'s voice, once: `
        + `${Math.round(progress.loaded / 1e6)} of ${Math.round(progress.total / 1e6)} MB` : "Getting the voice ready…");
    }
    showLoading();
  });
  ...
  /* The same, for a call long enough to be worth watching: `onProgress(done, total)`. */
  const callWatching = (onProgress, name, ...args) =>
    reading.ask({ type: "call", name, args }, [], onProgress).then((d) => d.result);
  ...
  // Click to read starts off (Kat, 23 September): a click that starts the
  // voice gets in the way of selecting and highlighting, which is most of what
  // a reader does with the mouse. Reading ▾ or the switches strip turns it on.
  const OFF_TO_START = new Set(["click_read"]);
  const switchOn = (name) => (OFF_TO_START.has(name) ? recall("mimick-" + name) === "1"
                                                      : recall("mimick-" + name) !== "0");
  ...
  /* Reading ▾ → What repeats (Kat, 23 September): the phrases and words the
   * document keeps using, from the desktop's themes.py, with how often and on
   * which pages. Choosing one finds it, so every place it comes up is lit. */
  async function showRepeats() {
    const mine = generation;
    status("Counting what repeats…");
    let found;
    try {
      found = await call("repeats");
    } catch (err) {
      if (mine === generation) status("Could not count what repeats: " + err.message);
      return;
    }
    if (mine !== generation) return;
    const list = $("repeats-list");
    list.replaceChildren();
    for (const [heading, entries] of [["Phrases", found.phrases], ["Words", found.words]]) {
      if (!entries.length) continue;
      list.append(Object.assign(document.createElement("h3"), { textContent: heading }));
      for (const { term, count, where } of entries) {
        const button = document.createElement("button");
        button.type = "button";
        button.append(Object.assign(document.createElement("b"), { textContent: term }),
                      Object.assign(document.createElement("span"), { textContent: `${count} times · p. ${where}` }));
        button.onclick = () => { $("repeats-dialog").close(); find.lookFor(term); };
        list.append(button);
      }
    }
    if (!list.children.length) list.append(Object.assign(document.createElement("p"), { className: "dim", textContent: "Nothing comes up often enough to count." }));
    status(openedStatus);
    $("repeats-dialog").showModal();
  }
  ...
  function readingMenu() {
    ...
    return [
      ...
      ...sleepMenu(),
      "-",
      { label: "What repeats…", enabled: built() && doc.sentences > 0, run: showRepeats },
  ...
  const convert = MimickConvert.create({
    call, status, offer, voiceKept: Voices.kept, speeds: SPEEDS,
    ready: () => !!doc?.sentences,
    title: () => doc.title,
  ...
  const notes = MimickNotes.create({
    call, status, offer, ask, unask, view, remember, recall, showMenu, copyText, placeBox,
    generation: () => generation,
    zoom: () => zoom,
  ...
  const tour = MimickTour.create({
    ...
    clickRead: () => switchOn("click_read"),
    // The Click to read steps light its switch, so the strip has to be open.
    showSwitches: () => openSwitches(true),
