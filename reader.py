...
#
# These are laid out as a PDF here, and from then on each *is* a PDF: the same
# reading, highlights, notes and Save a copy. A .txt keeps its line breaks as
# written, so a poem or a list reads as it looks; a blank line starts a paragraph.
# Word and EPUB are MuPDF's own (it lays them out as it would HTML); an .odt is
# turned into HTML here and set like a text file. Headings become the contents
# panel's entries. Images, footnotes and complicated tables come out rough.
...
# The file types document_to_pdf takes, and what one is called.
KINDS = {"docx": "a Word document", "odt": "an OpenDocument text file", "epub": "an EPUB book"}
